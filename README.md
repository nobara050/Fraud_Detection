...
- `output/` – logs and metrics produced during training/evaluation.  
- `content/` – images of results (ROC, confusion matrix, training curves, etc.).
...
- Result figures (and HTML visualizations) are stored in `content/` and can be used for reports or portfolio.

Example visualizations generated by the notebooks:

- **Schema graph overview**

  ![Schema graph](content/schema_graph.html)

- **Subgraph example around suspicious transactions**
